...
class PairCollector(child: Subscriber[WordPair]) extends Subscriber[Word] {
  ...
  private val wordOrdering: Ordering[Word] = Ordering.by(w => (w.size, w.value.hashCode))
  ...
  private def spawn(value: Word): Future[Unit] = Future {
    words.asScala.takeWhile(w => value.size + w.size >= biggestPair.get()).foreach(w => {
